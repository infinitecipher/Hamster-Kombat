...
(function () {
  ...
  console.log = function () {
    if (
      ...
      arguments[0].includes("[HamsterKombatBot]")
    ) {
  ...
  const settings = {
    ...
    minEnergyRefillDelay: 40000, // Минимальная задержка в миллисекундах для пополнения энергии (40 секунд)
    maxEnergyRefillDelay: 60000, // Максимальная задержка в миллисекундах для пополнения энергии (60 секунд)
  ...
  function performRandomClick() {
    ...
    if (!energyElement || !buttonElement) {
      ...
      if (retryCount >= settings.maxRetries) {
        console.log(
          `${logPrefix}Max retries reached, reloading page...`,
          styles.error
        );
        location.reload();
      } else {
    ...
    const energy = parseInt(
      energyElement.getElementsByTagName("p")[0].textContent.split(" / ")[0]
    );
    if (energy > settings.minEnergy) {
      ...
      console.log(
        `${logPrefix}Button clicked at (${randomX}, ${randomY})`,
        styles.success
      );
    } else {
  ...
  function clickThankYouBybitButton() {
    const thankYouButton = document.querySelector(
      ".bottom-sheet-button.button.button-primary.button-large"
    );
    if (thankYouButton) {
  ...
  setTimeout(() => {
    console.log(
      `${logPrefix}Script starting after 5 seconds delay...`,
      styles.starting
    );
